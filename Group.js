...
module.exports = class Group {
  ...
  async createGroup () {
    ...
    this.groupData = await asyncRequest('post', 'https://api.coolsms.co.kr/messages/v4/groups', { headers: { Authorization: getAuth() }, form: this.agent })
  ...
  addGroupMessage (messages) {
    ...
    return asyncRequest('put', `https://api.coolsms.co.kr/messages/v4/groups/${this.getGroupId()}/messages`, { headers: { Authorization: getAuth() }, form: { messages } })
  }
  /**
   * 추가된 메시지들을 발송 요청합니다.
   *
   ...
   */
  sendMessages () {
    return asyncRequest('post', `https://api.coolsms.co.kr/messages/v4/groups/${this.getGroupId()}/send`, { headers: { Authorization: getAuth() } })
  }
  /**
   * 그룹에 추가된 메시지들을 불러옵니다.
   *
  ...
  getMessageList (queryObject = { groupId: this.getGroupId() }) {
    ...
    return asyncRequest('get', `https://api.coolsms.co.kr/messages/v4/list${query}`, { headers: { Authorization: getAuth() } })
  }
  ...
  static deleteGroup (groupId) {
    return asyncRequest('delete', `https://api.coolsms.co.kr/messages/v4/groups/${groupId}`, { headers: { Authorization: getAuth() } })
  }
  /**
   * 그룹의 정보를 조회합니다.
   *
  ...
  static async getInfo (groupId) {
    return asyncRequest('get', `https://api.coolsms.co.kr/messages/v4/groups/${groupId}`, { headers: { Authorization: getAuth() } })
  }
  ...
  static async getMyGroupList (query) {
    ...
    return asyncRequest('get', `https://api.coolsms.co.kr/messages/v4/groups?${query}`, { headers: { Authorization: getAuth() } })
  }
  ...
  async deleteGroupMessages (messageId) {
    return asyncRequest('delete', `https://api.coolsms.co.kr/messages/v4/groups/${this.getGroupId()}/messages`, { headers: { Authorization: getAuth() }, form: { messageIds: [messageId] } })
  }
  ...
  static sendSimpleMessage (message = {}, agent = {}) {
    if (typeof message !== 'object') throw new Error('message 는 객체여야 합니다.')
    if (!message.autoTypeDetect && !message.type) throw new Error('autoTypeDetect 또는 type 을 입력해주세요.')
    return asyncRequest('post', `https://api.coolsms.co.kr/messages/v4/send`, { headers: { Authorization: getAuth() }, form: { message, agent } })
  }
  getGroupId () {
